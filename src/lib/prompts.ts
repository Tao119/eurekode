...
コード生成後、**生成したコードの具体的な内容に基づいた**クイズを出力してください。

【重要：コード固有のクイズを作成すること】
汎用的な質問ではなく、**実際のコードの具体的な部分**（関数名、変数名、処理内容）について質問してください。
...
【良い例 - このように質問する】
- 「calculateTotal関数で、itemsが空配列の場合、何が返されますか？」
- 「useStateで管理している'isLoading'がtrueになるのはいつですか？」
- 「fetchUserData関数がエラーをcatchした時、setErrorに何が渡されますか？」
- 「validateInput関数で、emailが空文字の場合どうなりますか？」

<!--QUIZ:{"level":1,"question":"[生成したコードの関数名/変数名を含む具体的な質問]","options":[{"label":"A","text":"[コードから読み取れる誤った答え]","explanation":"[なぜ違うかの説明]"},{"label":"B","text":"[正解]","explanation":"[コードのどこを見ればわかるか]"},{"label":"C","text":"[別の誤った答え]","explanation":"[なぜ違うかの説明]"}],"correctLabel":"B","hint":"[コードの何行目、どの関数を見るべきか]"}-->

【レベル別の質問内容】
- レベル1: 特定の関数や処理が何を返すか、どう動くか
- レベル2: 条件分岐時の挙動、エラー時の処理、変数の変化
- レベル3: なぜこの実装方法を選んだか、他の方法との違い

【クイズの厳守ルール】
- 必ず<!--QUIZ:...-->形式で出力
- **質問には必ず生成したコードの関数名・変数名・具体的な値を含める**
- options配列は3つ（各選択肢にexplanationを含める）
- correctLabelはA/B/Cをランダムに（毎回Aにしない）
- hintは「〜関数の戻り値を見てください」「〜行目のif文に注目」など具体的に
