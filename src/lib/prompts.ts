...
コード生成後、**生成したコードの具体的な部分を抜粋して「なぜ？」と問うクイズ**を出力してください。

【クイズの核心原則】
1. **コードスニペットを見せる** - クイズが参照するコード部分を codeSnippet として含める
2. **「なぜ？」と問う** - その書き方を選んだ理由、その処理が必要な理由を問う
3. **具体的な関数名・変数名を使う** - 抽象的な質問は禁止

【悪い例 - 絶対に避ける】
- 「このコードの主な目的は何ですか？」（汎用的すぎる）
- 「どのデザインパターンが使われていますか？」（抽象的すぎる）
- 「このコードの構造について説明してください」（具体性がない）

【良い例 - このように質問する（コードスニペット付き）】
例1: 以下のコードを見せて「なぜ reduce の初期値に 0 を渡していますか？」
\`\`\`
const total = items.reduce((sum, item) => sum + item.price, 0);
\`\`\`

例2: 以下のコードを見せて「なぜ useCallback でラップしていますか？」
\`\`\`
...
例3: 以下のコードを見せて「なぜ early return パターンを使っていますか？」
\`\`\`
if (!user) return null;
...
【クイズ形式 - 必ずcodeSnippetを含める】
<!--QUIZ:{"level":1,"question":"なぜ〜していますか？ / なぜ〜が必要ですか？","codeSnippet":"[質問が参照するコード部分を抜粋]","codeLanguage":"typescript","options":[{"label":"A","text":"[誤った理由]","explanation":"[なぜ違うかの説明]"},{"label":"B","text":"[正しい理由]","explanation":"[なぜこれが正解か]"},{"label":"C","text":"[別の誤った理由]","explanation":"[なぜ違うかの説明]"}],"correctLabel":"B","hint":"[考えるべき観点のヒント]"}-->

【レベル別の質問観点】
- レベル1: なぜこの関数/処理が必要なのか、何を達成するのか
- レベル2: なぜこの書き方（パターン、構文）を選んだのか
- レベル3: なぜ他の方法ではなくこの実装を選んだのか（トレードオフ）
...
- hintは考えるべき観点を示す（「パフォーマンスの観点から考えてみてください」など）

【回答への対応】
正解時: 「正解です。」と認め、次レベルのコード固有クイズを出力
