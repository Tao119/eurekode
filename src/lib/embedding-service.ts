...
export async function embedConversationMessages(
  ...
  try {
    ...
    if (!conversation) {
      console.warn(`[Embedding] Conversation not found: ${conversationId}`);
      return;
    }
    ...
    console.info(`[Embedding] Successfully embedded ${pairs.length} pairs for conversation: ${conversationId}`);
  } catch (error) {
    console.error(`[Embedding] Failed to embed conversation ${conversationId}:`, error);
    throw error;
  }
}
